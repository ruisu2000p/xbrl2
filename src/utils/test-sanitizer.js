/**
 * Simple test script for HTML sanitization
 * Run with: node src/utils/test-sanitizer.js
 */
const { sanitizeHtml, sanitizeHtmlPreserveTables, sanitizeHtmlEnhanced } = require('./htmlSanitizer');

const testHtmlWithTables = `
<div>
  <h1>Test HTML with Tables</h1>
  <p>This is a test paragraph</p>
  <table>
    <tr>
      <th style="vertical-align:middle;background-color:#cceeff;">Header 1</th>
      <th style="vertical-align:middle;background-color:#cceeff;">Header 2</th>
    </tr>
    <tr>
      <td style="vertical-align:middle;border-top-color:#a9a9a9;">Cell 1</td>
      <td style="vertical-align:middle;border-top-color:#a9a9a9;">Cell 2</td>
    </tr>
  </table>
  <ix:nonfraction contextRef="Prior1YearInstant_NonConsolidatedMember" decimals="-3" scale="3" format="ixt:numdotdecimal" name="jppfs_cor:LegalRetainedEarnings" unitRef="JPY">120,197</ix:nonfraction>
  <script>alert('dangerous');</script>
</div>
`;

console.log('=== Regular Sanitization (sanitizeHtml) ===');
console.log(sanitizeHtml(testHtmlWithTables));
console.log('\n');

console.log('=== Table-Preserving Sanitization (sanitizeHtmlPreserveTables) ===');
console.log(sanitizeHtmlPreserveTables(testHtmlWithTables));
console.log('\n');

console.log('=== Enhanced Sanitization (sanitizeHtmlEnhanced) ===');
console.log(sanitizeHtmlEnhanced(testHtmlWithTables));
